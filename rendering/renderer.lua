...
-- Remove a single ArrowTile by id from Renderer
...
-- Remove an operator by id from Renderer
function Renderer.remove_operator(id)
  operator_sprites[id] = nil
end

-- Set a new xScale value for all operators
...
function Renderer.draw_operators_arrowtiles()
  local draw_queue = {}
	for id, operator_sprite in pairs(operator_sprites) do
	  ...
	  -- screeny can be used as operator depth; smaller = top of screen = deeper = draw first
...
-- Operator animations
-- Add an action "block" consisted of an attacker and victim
...
-- Reset all attack animation-related variables
...
-- Initiate the first batch of attack animations
...
-- Callback on operator fade-out end after death
...
-- Return the number of attack animations active on screen
...
-- Play specific Spine2D animation for id
